...
function activateSubPanel(p, panels) {
	...
	jQuery('input[name="show_panel"]').val(showPanel);
...
jQuery(function($) {
	...
	$('form input[type=text], form input[type=password]').on('keypress', function (e) {
		var defaultButton = jQuery(this).parents('form').find('input[type=submit].default-submit-action');
...
function parseDocument($container) {
	...
	*/
	$container.find('input[data-reset-on-edit]').on('keyup', function() {
		jQuery(this.getAttribute('data-reset-on-edit')).val('');
	});
	...
	*/
	$container.find('.pagination .page-jump-form :button').click(function() {
		var $input = jQuery(this).siblings('input.inputbox');
		pageJump($input);
	});
	...
	$container.find('.pagination .page-jump-form input.inputbox').on('keypress', function(event) {
		if (event.which === 13 || event.keyCode === 13) {
			...
			pageJump(jQuery(this));
		}
	});

	$container.find('.pagination .dropdown-trigger').click(function() {
		var $dropdownContainer = jQuery(this).parent();
	...
	*/
	$container.find('table.responsive > tbody').not('.responsive-skip-empty').each(function() {
		var $items = jQuery(this).children('tr');
		if (!$items.length) {
			jQuery(this).parent('table:first').addClass('responsive-hide');
		}
	});
	...
	*/
	...
	$container.find('#navigation').each(function() {
		var $items = jQuery(this).children('ol, ul').children('li');
		if ($items.length === 1) {
			jQuery(this).addClass('responsive-hide');
		}
	});
