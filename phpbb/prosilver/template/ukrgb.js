...
function OnLoad() {
    var displayed = false;
    var exampleFx = new Fx.Tween('tweet-box', {
	    property: 'opacity',
	    duration: 400, 
	...
	$('tweet-box').setStyles({
		position: 'absolute',
                    top: $('tweet-button').getStyle('top')
		    });
	exampleFx.start(0,1);
    };
            
    if ($('tbox') != null) {
	$('tweet-close').addEvent('click', function(event) {
		event.stop();
		hide_tbox();
	    });
            
	$('tweet-button').addEvent('click', function(event) { 
		event.stop();
		if (displayed){
		    hide_tbox();
		}
    ...
    twttr.anywhere(function (T) {
	    T("#tbox").tweetBox({
			...
			width: 400,
			label: 'Tweet this!',
			defaultContent: "{TOPIC_TITLE} {BOARD_URL}viewtopic.php?f={S_FORUM_ID}&t={S_TOPIC_ID} #ukrgb",
			onTweet : function(plaintext, html) {hide_tbox()}
		});
	});
